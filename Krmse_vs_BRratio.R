## Sequentially increase branching ratio, compare estimation error

source("simetas.R")
source("Kestimate_functions.R")
library(RSpectra)

######### Generate 
set.seed(375) 
theta_beta = 5
a3 = 5; b3 = 2; # params for gaussian triggering
mu = 1; theta_K = 0.5; 
BigT = 1000

SvdRmse_avg <- c() # Average rmse over nreps
EmpRmse_avg <- c()
OlsRmse_avg <- c()
OlsRmse_avg2 <- c()
UnregRmse_avg <- c()
UnregRmse_avg2 <- c()

MeanNs <- c()
MeanBrRatio <- c()

nreps <- 5

set.seed(404)
test.weights <- c(0.05, 0.1, seq(0.15, .85, by = 0.1), 0.99, rep(1, 5))
be = 0.07 * BigT
bf = be

Grconds <- c()
UnregFails <- c()

#:length(test.weights)
for (tt in 4:length(test.weights)) {
  print(paste(tt, "/", length(test.weights)))
  if (test.weights[tt] >= 1) BigT = BigT * 0.9
  xs <- seq(0, BigT, length = 10000) # interval to interpolate estimates to for comparability
  SvdAvg <- double(length(xs))
  EmpAvg <- double(length(xs))
  OlsAvg <- double(length(xs))
  UnregAvg <- double(length(xs))
  
  SvdRmse <- c()
  EmpRmse <- c()
  OlsRmse <- c()
  UnregRmse <- c()
  UnregRmse2 <- c()
  
  SvdTimes <- c()
  EmpTimes <- c()
  OlsTimes <- c()
  UnregTimes <- c()
  EmpTimes <- c()
  
  zns <- c()
  brs <- c()
  rconds <- c()
  
  ## K \sim ba * N(bb, be) + bc * N(bd, bf)
  ## need to adjust as you increase magnitude so it doesn't become explosive
  bb = 0.25 * BigT; bd = 0.75 * BigT;
  ba <- test.weights[tt] / max(dnorm(xs, m = bb, s = be) + 0.7 * dnorm(xs, m = bd, s = bf))
  bc = 0.7 * ba
  myktest = ba * dnorm(xs, mean=bb, sd=be) + bc * dnorm(xs, mean=bd, sd=bf)
  if(max(myktest) > 1) next
  
  for (rep in 1:nreps) {
    z0 = simhawk(T = BigT, gmi = normprod, gxy = pointxy, gt = expgt, 
                 theta = list(mu = mu, K = theta_K, beta = theta_beta, b = 5,
                              ba=ba, bb=bb, bc=bc, bd=bd, be=be, bf=bf))
    n <- z0$n
    if (n > 3000) next
    zns <- c(zns, n)
    t <- z0$t
    myk = ba * dnorm(t, mean=bb, sd=be) + bc * dnorm(t, mean=bd, sd=bf)
    BW = BigT / 10
    
    #################################
    #### Pointwise estimates with SVD
    start <- Sys.time()
    G = matrix(0, nrow = n - 1, ncol = n - 1) # G[1,1] = g(t2-t1). deriv wrt beta1 for 1/lam2, 1/lam3.
    for (i in 1:(n-1)) {
      for (j in i:(n - 1)) {
        G[i,j] = dexp(t[j + 1] - t[i], rate = 1/theta_beta)
      }}
    Gs <- svd(G)
    
    # Select optimal regularization param (increase until sum > n - muT)
    rtests <- seq(0, 0.3, by = 0.01)[-1]
    reg_params <- rtests * Gs$d[1]
    
    Kprev <- c()
    for (ind in 1:length(reg_params)) {
      reg <- reg_params[ind]^2
      
      inlam0 <- tikh(Gs$u, Gs$d, Gs$v, rep(1, n - 1), reg)
      lambda0 <- c(mu, drop(1/inlam0$coef))
      
      Kest0 <- tikh(Gs$v, Gs$d, Gs$u, lambda0[2:n] - lambda0[1], reg)
      Kest0 <- c(drop(Kest0$coef), 0)
      
      Kest <- ksmooth(t, Kest0, kernel = "normal", b = BW, x.points = t)$y
      Kest <- pmax(0, Kest)
  
      if (sum(Kest) > (n - mu * T)) break
      else Kprev <- Kest
    }
    if (ind > 1) Kest <- Kprev
    SvdTimes <- c(SvdTimes, difftime(Sys.time(), start, units = "secs"))
    SvdAvg <- SvdAvg + ksmooth(t, Kest, kernel = "normal", b = BW, x.points = xs)$y
    SvdRmse <- c(SvdRmse, rmse(myk - Kest) / sum(myktest))
    
    #######################
    #### Empirical estimator
    start <- Sys.time()
    incr <- 7
    Kemp <- rep(0, n)
    for(i in 1:n){ 
      Kemp[i] = max(0, sum((t > t[i]) & (t < t[i] + incr)) - incr * mu) ## how many extra points occurred
    } 
    Kemp <- (Kemp / sum(Kemp)) * (n - mu * BigT)
    EmpAvg <- EmpAvg + ksmooth(t, Kemp, kernel = "normal", b = BW, x.points = xs)$y
    EmpTimes <- c(EmpTimes, difftime(Sys.time(), start, units = "secs"))
    Kemp <- ksmooth(t, Kemp, kernel = "normal", b = BW, x.points = t)$y
    EmpRmse <- c(EmpRmse, rmse(myk - Kemp) / sum(myktest))
    
    ###################################
    ## Unregularized solve
    start <- Sys.time()
    G = matrix(0, nrow = n - 1, ncol = n - 1) # G[1,1] = g(t2-t1). deriv wrt beta1 for 1/lam2, 1/lam3.
    for (i in 1:(n-1)) {
      for (j in i:(n - 1)) {
        G[i,j] = dexp(t[j+1] - t[i], rate = 1/theta_beta)
      }}
    rconds <- c(rconds, rcond(G))
    if (rcond(G) == 0) {
      warning(paste("Singular matrix with T=", BigT))
    } else {
      inlam2 <- solve(G, rep(1, n - 1))
      lambda2 <- c(mu, 1/inlam2)
      Kest4 <- solve(t(G),  lambda2[2:n] - lambda2[1])
      Kest4 <- ksmooth(t, c(Kest4, 0), kernel = "normal", b = BW, x.points = t)$y
      Kest4 <- pmax(0, Kest4)
      if (sum(Kest4) > 1e-1) {
        UnregRmse2 <- c(UnregRmse2, rmse(myk - Kest4) / sum(myktest))
        Kest4 <- (Kest4 / sum(Kest4)) * (n - mu * BigT)
        UnregAvg <- UnregAvg + ksmooth(t, Kest4, kernel = "normal", b = BW, x.points = xs)$y
        UnregTimes <- c(UnregTimes, difftime(Sys.time(), start, units = "secs"))
        UnregRmse <- c(UnregRmse, rmse(myk - Kest4) / sum(myktest))
      }
    }
    
    ###############################
    ############### Binned OLS: small discretization
    start <- Sys.time()
    I <- 10; delt <- 1.5 # discretization parameters
    bin.breaks <- seq(0, BigT + delt, by = delt)
    T2 <- length(bin.breaks) - 1
    W <- ceiling(T2 / I)
    true.ts = bin.breaks[seq(1, T2, by = I)] # for smoothing; the times each interval corresponds to
    
    hout <- hist(z0$t, breaks = bin.breaks, plot = F)
    binCounts <- hout$counts
    
    G <- matrix(0, nrow = T2 - 1, ncol = W)
    for (i in 2:T2) {
      upTo <- min(W, ceiling(i / I)) # which period the day is in
      for(j in 1:upTo) { # which periods precede bin i
        daysInJ <- seq((j-1) * I + 1, min(i - 1, j * I))
        daysInJ <- daysInJ[daysInJ < i] # some edge cases allow this
        G[i - 1, j] <- sum(dexp(i - daysInJ, 1/theta_beta) * binCounts[daysInJ])
      }
    }
    
    sG <- svds(G, k = 1, nu = 0, nv = 0)$d # max singular value
    reg_param <- 0.01 * sG
    L <- reg_param * diag(1, ncol = ncol(G), nrow = ncol(G))
    
    Kest0 <- (qr.solve(rbind(G, L), c(binCounts[-1] - mu * delt, rep(0, nrow(L)))))
    Kest0 <- c(Kest0, 0)
    Kest3 <- ksmooth(true.ts, Kest0, kernel = "normal", b = BW, x.points = t)$y
    Kest3 <- pmax(0, Kest3)
    
    OlsTimes <- c(OlsTimes, difftime(Sys.time(), start, units = "secs"))
    OlsAvg <- OlsAvg + ksmooth(t, Kest3, kernel = "normal", b = BW, x.points = xs)$y
    OlsRmse <- c(OlsRmse, rmse(myk - Kest3) / sum(myktest))
  }   
  
  trueNreps <- length(SvdRmse)
  if (trueNreps == 0) next
  UnregFails <- c(UnregFails, ifelse(length(UnregRmse) < trueNreps, mean(zns), 0))
  
  ####  Average RMSE/time for that set
  MeanNs <- c(MeanNs, mean(zns))
  MeanBrRatio <- c(MeanBrRatio, mean(myktest))
  Grconds <- c(Grconds, mean(rconds))
  
  SvdRmse_avg <- c(SvdRmse_avg, mean(SvdRmse))
  EmpRmse_avg <- c(EmpRmse_avg, mean(EmpRmse))
  OlsRmse_avg <- c(OlsRmse_avg, mean(OlsRmse))
  UnregRmse_avg <- c(UnregRmse_avg, mean(UnregRmse))
  UnregRmse_avg2 <- c(UnregRmse_avg2, mean(UnregRmse2))
  
  plot(t, myk, type = "l", xlab = "t", ylab = "K(t)", lwd = 2,
       ylim = range(c(0, Kest, Kest3, Kemp, Kest4, myk)), 
       main = paste("Mean K:", round(mean(myktest), 2)))
  lines(t, Kest, lty = 2, col = "red")
  lines(t, Kemp, lty = 2, col = "blue")
  lines(t, Kest4, lty = 2, col = "purple")
  lines(t, Kest3, lty = 2)
  legend("topright", lty = 2, col = c("red", "blue", "purple", "black"), cex = 0.95,
         legend = c("Regularized K", "Empirical K", "Unregularized", "Binned Least-Squares"))
}

UnregFails
par(mfrow=c(1,1), cex = 1.2)
plot(MeanBrRatio, SvdRmse_avg*100, col = "#1f77b4", pch = 16,
     main = "K ~ N(250,70) + N(750,70)",
     # main = "K ~ N(400,300)",
     ylab = "RMS Percent Error", xlab = "Mean Productivity",
     ylim = range(c(SvdRmse_avg*100, 2e-2)))
lines(MeanBrRatio, SvdRmse_avg*100, col = "#1f77b4")
points(MeanBrRatio[], UnregRmse_avg[]*100, pch = 0, lwd=2, col = "#ff7f0e")
lines(MeanBrRatio[], UnregRmse_avg[]*100, pch = 0, lwd=1, col = "#ff7f0e")

# points(MeanBrRatio[UnregFails > 0], UnregRmse_avg[UnregFails > 0], pch = 17, col = "red")
points(MeanBrRatio, EmpRmse_avg*100, pch = 2, lwd=2, col = "#2ca02c")
lines(MeanBrRatio, EmpRmse_avg*100, pch = 2, lwd=1, col = "#2ca02c")

points(MeanBrRatio, OlsRmse_avg*100, pch = 5, lwd=2, col = "#d62728")
lines(MeanBrRatio, OlsRmse_avg*100, pch = 5, lwd=1, col = "#d62728")

# points(MeanBrRatio, OlsRmse_avg2, pch = 5, col = "blue")
legend("topright", pch = c(16, 0, 2, 5), lty=0, bty="n", lwd = 2,
       cex = 1.15, col = c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"),
       legend = c("Regularized", "Unregularized", "Empirical", "Binned Least-Squares"))

